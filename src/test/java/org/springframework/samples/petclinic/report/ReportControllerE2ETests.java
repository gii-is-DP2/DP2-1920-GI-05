...
public class ReportControllerE2ETests {
	...
	void testListJudgeReports() throws Exception {
		mockMvc.perform(get("/judges/{judgeId}/reports", TEST_JUDGE_ID))
			.andExpect(status().isOk())
			.andExpect(model().attributeExists("reports"))
			.andExpect(view().name("reports/listJudge"));
	}
	
	@WithMockUser(username="owner1",authorities= {"owner"})
	@Test
	void testListOwnerReports() throws Exception {
		mockMvc.perform(get("/myReports/"))
			.andExpect(status().isOk())
			.andExpect(model().attributeExists("reports"))
			.andExpect(view().name("reports/listOwner"));
	}
	
	@WithMockUser(username="judge1",authorities= {"judge"})
	@Test
	void testGetCreateReport() throws Exception {
		mockMvc.perform(get("/judges/{judgeId}/reports/{tournamentsId}/new", TEST_JUDGE_ID, TEST_TOURNAMENT_ID))
			.andExpect(status().isOk())
